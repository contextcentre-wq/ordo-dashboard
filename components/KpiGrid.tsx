...
const KpiGrid: React.FC<KpiGridProps> = ({ metrics }) => {
  return (
    <div className="grid grid-cols-3 gap-x-4 gap-y-2 w-full bg-white px-4 py-3 md:px-5 md:py-4 rounded-2xl border border-gray-100/80 shadow-md shadow-sky-100/20">
      {metrics.map((metric, idx) => (
        <div key={idx} className="flex flex-col">
          <div className="flex items-center space-x-1">
            <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">{metric.label}</span>
